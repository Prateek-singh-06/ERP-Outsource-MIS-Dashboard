      ...
      <div className="relative z-10 p-6 pb-8">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-400">
                {title}
              </h3>
              {subtitle && (
        ...
        {/* Main value */}
        <div className="mb-4">
          <p className="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 flex items-baseline gap-1">
            {/* Show symbol/unit based on type */}
            {type === 'rupee' && <span className="text-3xl font-bold mr-1">₹</span>}
            <span>{typeof value === 'number' ? value.toLocaleString() : value}</span>
            {type === 'km' && <span className="font-bold ml-1">KM</span>}
            {type === 'hour' && <span className="font-bold ml-1">Hr</span>}
          </p>
        </div>
